...
if __name__ == "__main__":
    ...
    base_path = os.path.join(current_dir, "Slot_TWE_GSG_base.aedt")
    if not os.path.isfile(base_path):
        raise IOError("Cannot find base AEDT project: {0}".format(base_path))
    ScriptEnv.Initialize("Ansoft.ElectronicsDesktop")
    oDesktop.RestoreWindow()
    oDesktop.OpenProject(base_path)
    oProject = oDesktop.SetActiveProject("Slot_TWE_GSG_base")
    ...
    oProject = oDesktop.SetActiveProject("Slot_TWE_GSG_base")
    local_save_path = os.path.join(current_dir, "sim_result.aedt")
    oProject.SaveAs(local_save_path, True)
    ...
    # hu.change_local_variable(oDesign, "W", "800um")
    hu.change_local_variable(oDesign, "W", "{0}um".format(data.W))
